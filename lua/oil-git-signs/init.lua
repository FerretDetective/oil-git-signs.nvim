	...
	for line in vim.gsplit(stdout, "\n") do
		if line ~= "" then
			...
			if existing_entry == nil then
				...
				local existing_working_tree_score = M.options.status_priority[existing_entry.working_tree]
...
function M.nav_status(direction, count, statuses)
	...
	for _, extmark_item in ipairs(extmark_items) do
		...
		if (direction == "up" and row < cur_cursor_row) or (direction == "down" and row > cur_cursor_row) then
...
M.defaults = {
	...
	index = {
        ...
        [M.GitStatus.TYPECHANGED]      = { icon = "T", hl_group = "OilGitSignsIndexTypeChanged"    },
	...
	working_tree = {
        ...
        [M.GitStatus.TYPECHANGED]      = { icon = "T", hl_group = "OilGitSignsWorkingTreeTypeChanged"    },
	...
	status_classification = {
        ...
        [M.GitStatus.TYPECHANGED]      = "modified",
